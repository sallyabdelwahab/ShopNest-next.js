...
interface BallpitProps {
  className?: string;
  followCursor?: boolean;
  ballColors?: number[]; // <--- ألوان الكرات
  lightColor?: number;   // <--- لون الضوء
  [key: string]: any;
}

const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ballColors, lightColor, ...props }) => {
  ...
  useEffect(() => {
    ...
    // لو فيه ألوان جديدة حطيها
    if (spheresInstanceRef.current && ballColors?.length) {
      spheresInstanceRef.current.spheres.setColors(ballColors);
    }
    if (spheresInstanceRef.current && lightColor !== undefined) {
      spheresInstanceRef.current.spheres.light!.color.set(lightColor);
    }
